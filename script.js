...
let optHexModeDimensions;
let optHexModePixels;
let optHexModeBorder;
...
const updateHistoryView = () => {
    $('.historySelector').addClass('historySelectorHidden');
    $('.historySelector').removeClass('historySelectorSelected');
    $('.historySelector').prop('disabled', false);

    for (let i = 0; i < history.length; i++) {
        $(`.historySelector[data-state=${i}]`).removeClass('historySelectorHidden');
    }
...
const handleSelectHistoryState = (e) => {
    historyState = parseInt($(e.target).attr('data-state'));

    recordingHistory = false;
    unpackAndSetData(history[historyState]);
...
const toggleHexModeDimensions = () => {
    let width = parseInputValue($("#width").val(), baseDimensions);
    let height = parseInputValue($("#height").val(), baseDimensions);
...
let history = [];
let historyState = 0;
let recordingHistory = false; // to avoid recording history while recording history...
